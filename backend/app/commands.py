from .extensions import db
from .models import User
from werkzeug.security import generate_password_hash, check_password_hash
import config
from datetime import datetime

def ensure_admin_exists():
    """
    Checks if the default Administrator account exists.
    If not, creates it.
    If it exists, ensures the password matches the config (Single Source of Truth).
    """
    try:
        if not config.ADMIN_PASSWORDS:
            print("security warning: no ADMIN_PASSWORDS set, cannot manage admin.")
            return

        target_pwd = config.ADMIN_PASSWORDS[0]
        user = User.query.filter_by(username='Administrator').first()
        
        if not user:
            print("security: initializing default administrator account...")
            user = User(
                id='admin',
                username='Administrator',
                password_hash=generate_password_hash(target_pwd),
                password_hint="Initialized from Config",
                is_admin=True,
                created_at=int(datetime.now().timestamp())
            )
            db.session.add(user)
            db.session.commit()
            print("security: administrator account created.")
        else:
            # Check if password matches config
            if not check_password_hash(user.password_hash, target_pwd):
                print("security: admin password mismatch with config. Updating DB...")
                user.password_hash = generate_password_hash(target_pwd)
                db.session.commit()
                print("security: admin password updated.")
            
    except Exception as e:
        print(f"security init error: {e}")
